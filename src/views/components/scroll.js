...
class Scroll extends Component {
    ...
    componentDidMount() {
        ...
        document.querySelector('.wrapper > div').addEventListener('scroll', e => {
            document.querySelector('.scroll').style.transform = "translate(0, " + Math.min(e.target.offsetHeight - document.querySelector('.scroll').offsetHeight - 10, (e.target.scrollTop / (document.querySelector('.wrapper > div > div').offsetHeight + (document.querySelector('.wrapper > div > div').offsetHeight > 200 ? 50 : -50))) * 100) + "px)"
        }
