...
const DropdownStatus = ({ wrapper, setWrapper, array, closes, attribute, items }) => {
    ...
    return (
        <div className="order-dropdown" onClick={open}>
            ...
            <div className={show ? "btn-menu toggle" : "btn-menu"} >
                <ScrollBar height={90} style={{ position: 'relative' }} >

                    {/* <div style={{height: status.length * 22}}> */}
                    {status.filter(x => x.name.toLowerCase().includes(text.toLowerCase())).map((x, index) => (
                        ...
                        }} className={"btn-menu-list hoverTD " + (x.select === true ? 'select-btn' : '')} style={{ position: 'absolute', top: index * 22 }}><span className="menu-list-wrapper" style={{ pointerEvents: 'none' }}><span className={"order-tooltip  text-status"} style={{ pointerEvents: 'none' }} dangerouslySetInnerHTML={{ __html: searchUndreline(x.name) }}></span><span className='underline-status' style={{ background: x.color, top: 16 }}></span></span>
                        </div>
                    ))}
                    {/* </div> */}
                </ScrollBar>
            </div>
            <div className="elobaration-menu-block" style={(status.filter(x => x.name === showDropdown)[0]?.statusAttributes.length !== 0 && status.filter(x => x.name === showDropdown)[0]?.statusAttributes.length && showDropdown) ? { visibility: 'visible', opacity: 1, top: top } : {}}>
                <div className="elobaration-header-tooltip">Атрибут статуса: <span>{showDropdown}</span></div>
                <div className="elobaration-body">
                        <ScrollBar height={112} style={{ position: 'relative' }} hideBar={false} >
