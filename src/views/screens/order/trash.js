...
const Korobka = React.memo(({ count, onMouseEnter, onMouseLeave }) => (
  <span className={count === '0' ? "ico-wrap colorWhite icons": "ico-wrap icon-Exclude colorWhite icons"} onMouseEnter={count !== '0' ? onMouseEnter : undefined} onMouseLeave={onMouseLeave}>
...
const Additional = React.memo(({ count, hints, index }) => (
  <span className={count === '0' ? "ico-wrap colorWhite icons" : "ico-wrap icon-2 colorWhite icons"} onMouseEnter={e => {
    if (index !== 25 && count !== '0') {
      timer = setTimeout(() => {
...
const Konv = React.memo(({ count, index }) => (
...
function Trash({ data, rowHeight, changeCount, changeTop, refresh, zoom, changeRefresh, updateData, setModal, modal, changeIDList, updateLoading }) {
  ...
  useEffect(async () => {
    ...
    if (data.length <= getStart() + size && fetching) {
      setFetching(false)
      let dates = await fetch('http://192.168.0.197:3005/deleted', {
        method: 'POST',
        headers: {
  ...
  useEffect(async () => {
    if (refresh) {
      ...
      changeRefresh(false);
      fetch('http://192.168.0.197:3005/deleted', {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
          'Content-Type': 'application/json'
  ...
  const onMouseEnterHints = (e, text, x, flag = false, index) => {
    ...
    if (e.target.scrollWidth > e.target.offsetWidth && flag && text !== '' && !document.querySelector('.disableHover').classList.contains('disable-hover') && index !== 25) {
    ...
    } else if (!flag && text !== '' && !document.querySelector('.disableHover').classList.contains('disable-hover') && index !== 25) {
  ...
  const onMouseLeaveHints = (index) => {
    if (!document.querySelector('.disableHover').classList.contains('disable-hover') && index !== 25) {
      document.getElementById("tooltipBtn").style.animation = '';
      clearTimeout(timer);
    }
  }
              ...
              {status.length > 0 && Object.keys(column).map((x, i) => {
                ...
                if (x === "product" && column[x].show) {
                  return (
                    <th key={x} style={index === i ? { position: 'sticky', top: 24, zIndex: 11 } : { position: 'sticky', top: 24, zIndex: 3 }} onMouseEnter={e => setIndex(i)}>
                      <div className="wrap-hide">
                        <ProductDropdown setRange={setRange} refresh={refresh} keys={'goodsList'} search={search} showColumn={column[x].showContent} width={(column[x].width - 68)} countFolder={countFolder} folder={folders} wrapper={wrapper} onWrapper={onClickWrapper} />
                ...
                if (x === "send" && column[x].show) {
                  return (
                    <th key={x} style={index === i ? { position: 'sticky', top: 24, zIndex: 11 } : { position: 'sticky', top: 24, zIndex: 3 }} onMouseEnter={e => setIndex(i)}>
                      <DropdownLarge filter={'Пользователей'} data={users} resetSort={resetSort} setResetSort={setResetSort} setArr={updateData} search={search} keys={'sentByUserId'} setRange={setRange} refresh={refresh} showColumn={column[x].showContent} width={column[x].width - 27} wrapper={wrapper} onWrapper={onClickWrapper} />
                ...
                if (x === "change" && column[x].show) {
                  return (
                    <th key={x} style={index === i ? { position: 'sticky', top: 24, zIndex: 11 } : { position: 'sticky', top: 24, zIndex: 3 }} onMouseEnter={e => setIndex(i)}>
                      <DropdownLarge filter={'Пользователей'} data={users} resetSort={resetSort} setResetSort={setResetSort} setArr={updateData} search={search} keys={'editedByUserId'} setRange={setRange} refresh={refresh} showColumn={column[x].showContent} width={column[x].width - 27} wrapper={wrapper} onWrapper={onClickWrapper} />
                ...
                if (x === "date5" && column[x].show) {
                  return (
                    <th key={x} style={index === i ? { position: 'sticky', top: 24, zIndex: 11 } : { position: 'sticky', top: 24, zIndex: 3 }} onMouseEnter={e => setIndex(i)}>
                      <DropdownLarge filter={'Пользователей'} data={users} resetSort={resetSort} setResetSort={setResetSort} setArr={updateData} search={search} keys={'acceptedByUserId'} setRange={setRange} refresh={refresh} showColumn={column[x].showContent} width={column[x].width - 27} wrapper={wrapper} onWrapper={onClickWrapper} />
          ...
          {data.length > 0 && <tbody className='disableHover' style={{ marginTop: 5 }}>
            ...
            {data.slice(getStart(), getStart() + visible + 1).map((row, rowIndex) => (
              <tr
                ...
                onMouseEnter={(getStart() + rowIndex === 25) ? e => {
                  timer = setTimeout(() => {
                ...
                onMouseLeave={(getStart() + rowIndex === 25) ? onMouseLeaveHints : undefined}
                    ...
                    if (x === 'id' && column[x].show && column[x].showContent) {
                      ...
                      return (
                        ...
                        <td key={x} className="status-table" style={{
                          ...
                          <div className="new-zakaz color-form2" style={{ background: row.status.color, overflow: 'hidden', textOverflow: 'ellipsis', width: column['status'].width }} onMouseEnter={e => onMouseEnterHints(e, row.status.name, x, true, getStart() + rowIndex)}
                            onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)}>
                            {row.status.name}
                          </div>
                        </td>
                      ...
                      return (
                        <td key={x} className="prro-colum" style={{ maxWidth: 73, minWidth: 73 }}>
                          {<><span style={{ display: 'inline-block', overflow: 'hidden', textOverflow: 'ellipsis', width: column['ppo'].width }} className={'prro-number'} onMouseEnter={e => onMouseEnterHints(e, row.ppo, x, false, getStart() + rowIndex)}
                            onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)}>{row.ppo}</span>
                            <span className={"colorWhite icons " + row.count_ppo} onMouseEnter={e => onMouseEnterHints(e, ppo.filter(x => x.icon?.includes(row.count_ppo))[0].hint === 'sms' ? hints.sms : hints.mail, x, false, getStart() + rowIndex)}
                              onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)}>
                            </span></>}
                        </td>
                      )
                      ...
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, row.customer, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.customer}</td>
                      )
                    }
                    else if (x === "localization" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} className={row.country === "Глобально" ? "country-block " + country[row.country] : "country-block flags ua "} onMouseEnter={e => onMouseEnterHints(e, row.country, x, false, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >
                      ...
                      return (
                        <td key={x} className="tel-colum" style={{ pointerEvents: 'all' }} >
                          ...
                          {<div className={'tel'}
                            ...
                            })[0]?.title ?? '', x, false, getStart() + rowIndex)}
                            onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >
                          ...
                          {row.count_message !== "" && <Konv count={row.count_message} index={getStart() + rowIndex} />}
                        </td>
                      )
                    }
                      ...
                      return (
                        <td key={x} className={"colum-pay icons colorWhite " + (row.pay)} onMouseEnter={e => onMouseEnterHints(e, pay.filter(x => x.icon?.includes(row.pay))[0].title, x, false, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >
                        </td>
                      )
                    }
                      ...
                      return (
                        <td key={x} className={"colum-delivery icons " + (row.delivery)} onMouseEnter={e => onMouseEnterHints(e, deliveries.filter(y => y.icon?.includes(row.delivery))[0].title, x, false, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >
                        </td>
                      )
                    }
                      ...
                      return (
                        <td key={x} className="addres-block" style={styles} onMouseEnter={e => onMouseEnterHints(e, row.address, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.address}</td>
                      )
                    }
                      ...
                      return (
                        <td key={x} className="colum-ttn">
                          {<div className="ttn-position">
                            ...
                            <Korobka count={2} onMouseEnter={e => onMouseEnterHints(e, 'Остался 2 дн до платного хранения', x, false, getStart() + rowIndex)}
                              onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} />
                          </div>}
                        </td>
                      ...
                      return (
                        <td key={x} onMouseEnter={e => onMouseEnterHints(e, row.ttn_status, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} style={styles} >{row.ttn_status}</td>
                      )
                    }
                    else if (x === "ttn_user" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} onMouseEnter={e => onMouseEnterHints(e, row?.viewedByUser?.name, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} style={styles} >{row?.viewedByUser?.name}</td>
                      )
                    }
                    else if (x === "office" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} className="otdel-block" onMouseEnter={e => onMouseEnterHints(e, row?.department?.name, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} style={styles} >{row?.department?.name}</td>
                      )
                    }
                      ...
                      return (
                        <td key={x} className="date-time otkrit time" onMouseEnter={e => onMouseEnterHints(e, row.hints_open, x, false, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >
                      ...
                      return (
                        <td key={x} className="date-time acceptza time" onMouseEnter={e => onMouseEnterHints(e, row.hints_success, x, false, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >
                      ...
                      return (
                        <td key={x} onMouseEnter={e => onMouseEnterHints(e, row?.sentByUser?.name, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} style={styles} >{row?.sentByUser?.name}</td>
                      )
                    }
                    else if (x === "change" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x}  onMouseEnter={e => onMouseEnterHints(e, row?.editedByUser?.name, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} style={styles} >{row?.editedByUser?.name}</td>
                      )
                    }
                      ...
                      return (
                        <td key={x}  onMouseEnter={e => onMouseEnterHints(e, row?.acceptedByUser?.name, x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} style={styles} >{row?.acceptedByUser?.name}</td>
                      )
                    }
                      ...
                      return (
                        <td key={x} className="date-block time" onMouseEnter={e => onMouseEnterHints(e, row.hints_send, x, false, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >
                      ...
                      return (
                        <td key={x} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.site, x), x, false, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} style={styles} >{row.domen}</td>)
                    }
                        ...
                        <TD key={x} className={'ip-block'}>
                          {<div className="ip-block-position">
                            ...
                            <span className="ip-icons-position">
                              <span className="flags" onMouseEnter={e => onMouseEnterHints(e, row.country, x, false, getStart() + rowIndex)}
                                onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{country[row.country_order]}</span>
                              <span className={row.type_device + " icons colorWhite"} onMouseEnter={e => onMouseEnterHints(e, device.filter(x => x.icon?.includes(row.type_device))[0].title, x, false, getStart() + rowIndex)}
                                onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} ></span>
                              <span className={row.type_os + " icons colorWhite"} onMouseEnter={e => onMouseEnterHints(e, system.filter(x => x.icon?.includes(row.type_os))[0]?.title || '', x, false, getStart() + rowIndex)}
                                onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)}></span>
                              <span className={row.type_browser + " icons colorWhite "} onMouseEnter={e => onMouseEnterHints(e, browser.filter(x => x.icon?.includes(row.type_browser))[0].title, x, false, getStart() + rowIndex)}
                                onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)}></span>
                            </span>
                          </div>}
                        </TD>
                      ...
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.utm_source, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.utm_source}</td>
                      )
                    }
                    else if (x === "utm2" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.utm_medium, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.utm_medium}</td>
                      )
                    }
                    else if (x === "utm3" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.utm_term, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.utm_term}</td>
                      )
                    }
                    else if (x === "utm4" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.utm_content, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.utm_content}</td>
                      )
                    }
                    else if (x === "utm5" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.utm_campaign, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.utm_campaign}</td>
                      )
                    }
                    else if (x === "additional_1" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_1, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_1}</td>
                      )
                    }
                    else if (x === "additional_2" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_2, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_2}</td>
                      )
                    }
                    else if (x === "additional_3" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_3, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_3}</td>
                      )
                    }
                    else if (x === "additional_4" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_4, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_4}</td>
                      )
                    }
                    else if (x === "additional_5" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_5, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_5}</td>
                      )
                    }
                    else if (x === "additional_6" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_6, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_6}</td>
                      )
                    }
                    else if (x === "additional_7" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_7, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_7}</td>
                      )
                    }
                    else if (x === "additional_8" && column[x].show && column[x].showContent) {
                      return (
                        <td key={x} style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_8, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_8}</td>
                      )
                    }
                    else if (x === "additional_9" && column[x].show && column[x].showContent) {
                      return (
                        <td style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_9, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_9}</td>
                      )
                    }
                    else if (x === "additional_10" && column[x].show && column[x].showContent) {
                      return (
                        <td style={styles} onMouseEnter={e => onMouseEnterHints(e, lightHints(row.additional_field_10, x), x, true, getStart() + rowIndex)}
                          onMouseLeave={e => onMouseLeaveHints(getStart() + rowIndex)} >{row.additional_field_10}</td>
                      )
                    }
                  })
                }
