...
function Order({ data, rowHeight, changeCount, changeTop, refresh, zoom, changeRefresh, updateData, setModal, modal, changeIDList, updateLoading }) {
        ...
        {status.length > 0 && <table style={{ width: 0, outline: 'none' }} tabIndex={-1} className={'crm-table speed'}>
          <thead tabIndex={-1}>
                  ...
                  return (
                    <TH style={{
                      ...{
                        ...
                        position: 'sticky',
                        top: 0, zIndex: 3, backgroundColor: (i % 2 !== 0 && i !== 2 && i !== 1) || i === 0 ?  '#F1F1F1': '#FFF'
                      }, ...(i < 2 && { left: i === 1 ? width + 16 : 16, zIndex: 45 })
                    }}
                      className="header-id"
                      hint={hints.id}
                      key={key}
            ...
            <tr className="crm-input" key={'crm-input'}>
              ...
              {status.length > 0 && Object.keys(column).map((x, i) => {
                ...
                if (x === "status" && column[x].show) {
                  return (
                    <th key={x} style={index === i ? { position: 'sticky', top: 24, left: 16 + width, zIndex: 45 } : { position: 'sticky', top: 24, left: 15 + width, zIndex: 45 }} onMouseEnter={e => setIndex(i)}>
                      <DropdownLarge
                        filter={'Статусов'}
                        data={status}
                        resetSort={resetSort}
            ...
            <tr key={'crm-wrapper'} style={{ height: 0, zIndex: -1, position: 'sticky', top: 24, left: 0 }} className="table-header">
              ...
              {Object.entries(column).slice(3).map(([key, value], i) => {
                      ...
                      return (
                        <td key={x} className="product-colum">
                          {<>
                            ...
                            <span style={{ width: column['product'].width - 53, display: 'block', overflow: "hidden", textOverflow: 'ellipsis' }} className="max-length -product" onMouseEnter={e => onMouseEnterHints(e, `
